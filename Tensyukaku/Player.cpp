   ...
   for (auto ite = g.GetOS()->List()->begin(); ite != g.GetOS()->List()->end(); ite++)
   {
      ...
      case ObjectBase::OBJECTTYPE::RECOVERYBLOCK:
         ...
         if (IsHit(*(*ite)) == true) {
            ...
            _life = 3;
            //SE
            PlaySoundMem(_se["Recovery"], DX_PLAYTYPE_BACK, true);
            for (int i = 0; i < RECOVERY_PARTICLE_QTY; i++)
            {
               std::pair<int, int> xy = std::make_pair(_x, _y);
               std::pair<double, double> dxy = std::make_pair(((rand() % RECOVERY_PARTICLE_RANDOMX1) - RECOVERY_PARTICLE_RANDOMX2) / RECOVERY_PARTICLE_RANDOMX3, ((rand() % RECOVERY_PARTICLE_RANDOMY1) - GAGEMAX_PARTICLE_RANDOMY2) / GAGEMAX_PARTICLE_RANDOMY3);
               auto rp = new RecoveryParticle(xy, dxy);
               g.GetOS()->Add(rp);
            }
         }
         break;
